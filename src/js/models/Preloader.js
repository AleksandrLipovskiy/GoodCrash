...
export class Preloader {
  ...
  _loadingSuccessScene () {
    return new Promise((resolve, reject) => {
      const print = async () => {
        await animate.printText(this.DOM.preloaderCtx, this.text["success preload"], this.timePrinted);
      }

      print().then(() => {
        resolve();
      }).catch((err) => { console.log(`Preloader._loadingSuccessScene: ${ err }`) });
    });
  }
  ...
  _glitchScreen () {
    return new Promise((resolve, reject) => {
      ...
      const print = async () => {
        await animate.printText(this.DOM.preloaderCtx, this.text["error preload"], this.timePrinted);
      }

      print().then(() => {
        resolve();
      }).catch((err) => { console.log(`Preloader._glitchScreen: ${ err }`) });
    });
  }
  ...
  _loadingCrashIcon () {
    return new Promise((resolve, reject) => {
      ...
      const drawCrashIcon = async () => {
        await animate.deleteText(this.DOM.preloaderCtx, this.text["error preload"], this.timePrinted);
        await animate.printText(this.DOM.preloaderCtx, this.text["crash preload"], this.timePrinted);
      }

      drawCrashIcon().then(() => {
        resolve();
      }).catch((err) => { console.log(`Preloader._loadingCrashIcon: ${ err }`) });
    });
  }
  ...
  _loadingGoodCrashIcon () {
    return new Promise((resolve, reject) => {
      const addClass = () => {
        ...
        this.DOM.preloaderCtx.dataset.text = '';
        this.DOM.preloaderCtx.dataset.text += this.text["goodcrash preload"]["0"];
      }

      const drawGoodCrashIcon = async () => {
        await animate.deleteText(this.DOM.preloaderCtx, this.text["crash preload"], this.timePrinted);
        await addClass();
        await animate.printText(this.DOM.preloaderCtx, this.text["goodcrash preload"], this.timePrinted);
      }
